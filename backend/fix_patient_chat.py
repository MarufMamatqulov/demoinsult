"""
Fix script for patient_chat.py to use the new OpenAI helper.
"""
...
from backend.api.patient_chat import router, get_chat_response, get_assessment_advice
from backend.utils.openai_helper import create_chat_completion

# Monkey patch the functions to use our helper
original_get_chat_response = get_chat_response
original_get_assessment_advice = get_assessment_advice

async def patched_get_chat_response(request):
    """Patched version of get_chat_response that uses our OpenAI helper."""
    ...
    try:
        ...
        # Call OpenAI API using our helper
        ...
        if any(keyword in str(request.messages[-1].content).lower() for keyword in 
            ...
            # Use our helper for advice generation too
        ...
        from backend.api.patient_chat import ChatResponse
...
async def patched_get_assessment_advice(request):
    """Patched version of get_assessment_advice that uses our OpenAI helper."""
    from fastapi import HTTPException
    import logging
    import json
...
get_chat_response.__code__ = patched_get_chat_response.__code__
get_assessment_advice.__code__ = patched_get_assessment_advice.__code__

print("Successfully patched patient_chat.py to use the OpenAI helper")
